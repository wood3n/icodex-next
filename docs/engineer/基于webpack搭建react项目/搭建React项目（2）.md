---
title: 搭建 React 项目（2）
description: "之前通过一些 webpack 的简单配置，项目可以使用 React 来编写基本组件了，但是每次编写组件，都需要重新执行命令打包，然后再手动在浏览器中打开页面才能看到开发结果，这是十分影响开发效率的，我们要的效果启动项目运行，在开发过程中使用保存文件，然后页面能自动更新，为此需要配置 HMR 和 。"
keywords: ["开发模式", "development", "production", "针对不同模式的配置", "不同模式的切换", "wds", "hmr", "react"]
tags: ["开发模式", "development", "production", "针对不同模式的配置", "不同模式的切换"]
---
