---
title: 定时器
description: "从 whatwg 规范定义的内容来看，无论是还是都会经过一个初始化定时器的步骤，并且它们俩用的都是同一套算法，同一个定时器任务列表，见 —— ，大致的步骤如下："
keywords: ["timers", "in", "parallel", "settimeout", "实现", "setinterval", "requestanimationframe", "机制"]
tags: ["timers", "in", "parallel", "settimeout", "实现"]
---
